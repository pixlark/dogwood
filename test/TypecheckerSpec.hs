...
testTypecheck source = (show <$$>) $ fmap stripCallStacks $ runEff $ runErrors $ runLog noOpLogger do
...
spec = do
  describe "the Typechecker module" do
    it "typechecks basic expressions" do
      testTypecheck "let main = fn() { let x: void = void; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet x: void = void;\n} : void;\n"

      testTypecheck "let main = fn() { let x: int = 1 + 2; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet x: int = (1 + 2 : int);\n} : void;\n"

      testTypecheck "let main = fn() { let x: bool = !false; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet x: bool = (!false : bool);\n} : void;\n"

      testTypecheck "let main = fn() { let x: int = 5 != 2; };"
        `shouldSatisfyM` isErrorKind (TypeMismatch "int" "bool")

    it "typechecks body expressions" do
      testTypecheck "let main = fn() { {}; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\n{\n} : void;\n} : void;\n"

      testTypecheck "let main = fn() { let x = { 5; }; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet x: void = {\n5;\n} : void;\n} : void;\n"

      testTypecheck "let main = fn() { let x = { 5 }; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet x: int = {\n5\n} : int;\n} : void;\n"

      testTypecheck "let main = fn() { let x = { true; 5; }; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet x: void = {\ntrue;\n5;\n} : void;\n} : void;\n"
    ...
    it "typechecks bound variables" do
      testTypecheck "let main = fn() { let x: int = foo; };"
        `shouldSatisfyM` isErrorKind (UnboundVariable "foo")

      testTypecheck "let main = fn() { {let foo: int = 5; let bar: int = foo;}; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\n{\nlet foo: int = 5;\nlet bar: int = (foo : int);\n} : void;\n} : void;\n"
    ...
    it "can infer the type of a variable declaration" do
      ...
      testTypecheck "let main = fn() { let print = builtin print; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet print: fn(any) -> void = builtin print : fn(any) -> void;\n} : void;\n"

      testTypecheck "let main = fn() { let x = 5; let y = x; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet x: int = 5;\nlet y: int = (x : int);\n} : void;\n"

    it "typechecks function calls" do
      testTypecheck "let main = fn() { let x: int = 5; x(15, 16); };"
        `shouldSatisfyM` isErrorKind (CallingNonFunction "int")

      testTypecheck "let main = fn() { let f = fn(a: int, b: int) -> bool: true; let y = f(5, 6); };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet f: fn(int, int) -> bool = fn(a: int, b: int) -> bool: true;\nlet y: bool = (f(5, 6) : bool);\n} : void;\n"

      testTypecheck "let main = fn() { let f = fn(a: int, b: int) -> bool: true; f(true, 6); };"
        `shouldSatisfyM` isErrorKind (TypeMismatch "int" "bool")

      testTypecheck "let main = fn() { let f = fn(a: int, b: int) -> void {}; f(1); };"
        `shouldSatisfyM` isErrorKind (WrongArgumentCount 2 1)

    it "typechecks if expressions" do
      testTypecheck "let main = fn() { if true void else if false void; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\n(if true void else (if false void else void) : void) : void;\n} : void;\n"

      testTypecheck "let main = fn() { if 15 void else if false void; };"
        `shouldSatisfyM` isErrorKind (TypeMismatch "bool" "int")

      testTypecheck "let main = fn() { let x = if true 15 else 16; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet x: int = (if true 15 else 16) : int;\n} : void;\n"

      testTypecheck "let main = fn() { let x = if true 15 else true; };"
        `shouldSatisfyM` isErrorKind (TypeMismatch "int" "bool")

      testTypecheck "let main = fn() { if true { 15; }; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\n(if true {\n15;\n} : void else void) : void;\n} : void;\n"
    ...
    it "typechecks loops" do
      testTypecheck "let main = fn() { loop { 15 } };"
    ...
    it "typechecks lexical scopes" do
      testTypecheck "let main = fn() { let y = { let x: int = 5; { let x: bool = false; x || false } x + 5 }; };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet y: int = {\nlet x: int = 5;\n{\nlet x: bool = false;\n((x : bool) || false : bool)\n} : bool\n((x : int) + 5 : int)\n} : int;\n} : void;\n"

    it "typechecks lambda expressions" do
      testTypecheck "let main = fn() { let x = 5; let f = fn() -> int: x; };"
        `shouldSatisfyM` isErrorKind (UnboundVariable "x")

      testTypecheck "let main = fn() { let f = fn() -> int: void; };"
        `shouldSatisfyM` isErrorKind (TypeMismatch "void" "int")

      testTypecheck "let main = fn() { let f = fn(x: int) -> int: x; let y = f(1); };"
    ...
    it "typechecks mutually recursive toplevel functions" do
      let source =
          ...
          expected =
            ...
            |]
      testTypecheck source `shouldReturn` Right (expected `Text.append` "\n")
    ...
    it "typechecks new operator" do
      ...
      testTypecheck "let main = fn() { let x = new int(5); };"
        `shouldReturn` Right "let main: fn() -> void = fn() -> void {\nlet x: &int = new int(5);\n} : void;\n"
      ...
      testTypecheck "let main = fn() { let x = new fn() -> void; };"
        `shouldSatisfyM` isErrorKind NonConstructibleType

      -- Wrong constructor argument count
      testTypecheck "let main = fn() { let x = new int(5, 6); };"
        `shouldSatisfyM` isErrorKind (WrongConstructorCount 1 2)

    it "typechecks dereference operator" do
      -- Cannot dereference a value type
      testTypecheck "let main = fn() { let x: int = 5; let y = *x; };"
        `shouldSatisfyM` isErrorKind (DereferencedValueType "int")

      -- Cannot dereference a literal
      testTypecheck "let main = fn() { let y = *5; };"
        `shouldSatisfyM` isErrorKind (DereferencedValueType "int")

    it "typechecks lvalue dereference" do
      -- Cannot dereference a value type in an lvalue context
      testTypecheck "let main = fn() { let x: int = 5; *x = 10; };"
        `shouldSatisfyM` isErrorKind (DereferencedValueType "int")
