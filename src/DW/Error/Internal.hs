...
import Debug.Trace
import Effectful (Dispatch (..), DispatchOf, Eff, Effect, runPureEff, (:>))
...
-- newtype instance StaticRep (Errors e) = Errors [Err]

-- runErrors :: Eff (Errors e : es) a -> Eff es (Either [Err] a)
-- runErrors action = evalStaticRep (Errors []) action
